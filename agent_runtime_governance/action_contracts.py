...
class ActionContract:
    ...
    def __post_init__(self) -> None:
        ...
        if receipt_schema is not None:
            object.__setattr__(
                self, "receipt_schema", _freeze_mapping(receipt_schema)
            )
    ...
    def bind(
        ...
        *,
        ...
        """Bind an isolated parameter snapshot and governance identity."""
        return BoundAction(
            self,
            parameters,
    ...
    def to_dict(self) -> dict[str, Any]:
        """Return the full versioned persistence form.
        ...
        should use :meth:`to_evidence_dict` instead.
        """
        return {
    ...
    def from_dict(cls, value: Mapping[str, Any]) -> ActionContract:
        ...
        try:
            ...
            raise ContractValidationError("serialized action contract", str(exc)) from exc
    ...
    def __init__(
        ...
        *,
        ...
        if contract.precondition_requirements and precondition_digest is None:
            raise ValueError(
                "precondition_digest is required by this action contract"
            )
        ...
        _initialize_bound_action(
            ...
            principal_digest=_principal_digest(identity_issuer, principal),
            tenant_digest=_identifier_digest(_TENANT_DOMAIN, tenant),
            policy_version=policy_version,
            policy_digest=policy_digest,
            precondition_digest=precondition_digest,
...
def _prepare_parameters(
    ...
    validated = _validate_parameters(strict, contract.parameters_schema)
...
def _principal_digest(issuer: str, subject: str) -> str:
    return _sha256(
        _canonical_bytes(
            {
                "domain": _PRINCIPAL_DOMAIN,
                "version": _ENVELOPE_VERSION,
                "issuer": issuer,
                "subject": subject,
            },
            label=_PRINCIPAL_DOMAIN,
        )
    )


def _identifier_digest(domain: str, value: str) -> str:
    return _sha256(
        _canonical_bytes(
            {"domain": domain, "version": _ENVELOPE_VERSION, "identifier": value},
            label=domain,
        )
    )
...
def _validate_parameters(
    value: dict[str, Any], schema: Mapping[str, Any]
) -> dict[str, Any]:
    validator = Draft202012Validator(_thaw(schema))
    errors = sorted(
    ...
    *,
    ...
    if type(value) is float:
        ...
        if value == 0.0 and math.copysign(1.0, value) < 0:
            raise ContractValidationError(path, "negative zero is ambiguous in RFC 8785")
        return value
    ...
    try:
        if mapping:
            ...
            for key, item in value.items():
                ...
                if key in result:
                    raise ContractValidationError(path, "duplicate object keys are invalid")
                _normalize_value(
                    key,
                    path=path,
                    depth=depth + 1,
...
def _require_envelope(
    value: Mapping[str, Any], *, domain: str, label: str
) -> None:
